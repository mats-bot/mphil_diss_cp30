...
def create_timeseries(HIS_norm, peak_hour, AM_min_hour, PM_min_hour, P, A, M):
    """
    Returns an hourly timeseries for yearly data entry from FES regional breakdown based on peaks/
    minimums.
    ...
    for i in range(3):
        ...
        if h_end > h_start:
            ...
            hours = np.concatenate([np.arange(h_start, len(HIS_norm)), np.arange(0, h_end)])
