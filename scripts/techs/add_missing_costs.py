...
battery_capital_cost = (
    np.mean([60, 70]) / 0.7263 * c15 * 1000
)  # euro/MW * pound/euro|2015 https://www.exchangerates.org.uk/EUR-GBP-spot-exchange-rates-history-2015.html
battery_energy_storage_cost = (
    np.mean([230, 610]) / 0.7263 * c15 * 1000
)  # euro/MWh * pound/euro|2015
...
pumped_hydro_energy_cap_max = (
    pumped_hydro_power_cap * 8
)  # MWh, 700 is avg pumped hydro cap, assumed 8 hrs but large variation since one 1.7 GW and rest <0.5 GW
...
# 2015 costs
pumped_hydro_capital_cost = (
    (np.mean([260, 560]) + np.mean([220, 460])) / 0.7263 * c15 * 1000
)  # euro/MW * pound/euro|2015  https://www.exchangerates.org.uk/EUR-GBP-spot-exchange-rates-history-2015.html
pumped_hydro_energy_storage_cost = (
    np.mean([10, 20]) / 0.7263 * c15 * 1000
)  # euro/MWh * pound/euro|2015
pumped_hydro_om_fixed = (
    11 / 80 * 1 / 0.7263 * c15 * 1000
)  # euro/MW/yr * pound/euro|2015
...
# 2015 costs
LAES_om_fixed = (
    11.2 * 1 / 0.7263 * c15 * 1000
)  # euro/MW/yr * pound/euro|2015 #https://www.exchangerates.org.uk/EUR-GBP-spot-exchange-rates-history-2015.html
...
storage_data = {
    ...
    "storage_loss": [
        battery_storage_loss,
        pumped_hydro_storage_loss,
        CAES_storage_loss,
        LAES_storage_loss,
    ],
    "charge_rate": [
    ...
    "energy_cap_equals": [
        battery_power_cap,
        pumped_hydro_power_cap,
        CAES_power_cap,
        LAES_power_cap,
    ],
    "storage_cap_equals": [
        battery_energy_cap_max,
        pumped_hydro_energy_cap_max,
        CAES_energy_cap_max,
        LAES_energy_cap_max,
    ],
    ...
    "om_prod": [None, pumped_hydro_om_prod, CAES_om_prod, None],
...
# costs in 2014 GBP
nuclear_capex = (
    (240 + 4100 + 11500000 / 3300000) * c14 * 1000
)  #  £/MW includes insurance
nuclear_om_annual = (
    72900 + 500 + 10000
) * c14  # £/MW/yr  includes connection/use of system and insurance
...
co2ts = (
    18 * c15
)  # £/tCO2, 2015 cost https://ukerc.rl.ac.uk/publications/technical_report/CCS_CC1026_14.pdf
...
fuel_cost_row = pd.Series(
    {tech: fuel_costs.get(tech, 0) for tech in df.columns}, name="fuel_cost"
)
